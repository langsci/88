...
The first two examples still lack the vowel harmony to which these suffixes are subject in modern Hungarian. The complex suffixes are based on the relational noun \textit{bél} ‘innards’ and are to be analyzed as shown in \tabref{tab:DevHung} (cf. also \citet[118-121]{Kahr1976}):
...
We should pause here for a moment to pin down this syntactic structure and compare it with the one we posited in the preceding section as the basis of adpositions which govern the genitive (\REF{ex:F4}). The common denominator of the above constructions may be represented as in \figref{ex:F5}.
...
Similar phenomena may be found in (non-Persian) Iranian languages. If the adposition becomes affixal, a case circumfix is the result. Such exist in Mangarayi (see \citet[57-59]{Merlan1982}). The case paradigm of the masculine noun \textit{malam} ‘man’ is as in \tabref{tab:Mangarayi}.
...
\ea\label{ex:}
\langinfo{Efik}{}{\citet[369]{Welmers1973}}\\
\gll dá  íkwâ  dí!  \\
 take  knife  come  \\
 \glt ‘Bring a knife!’   
\z
...
\caption{Some interrelated grammaticalization channels of cases}
\end{figure}
...
\ea\label{ex:}
\langinfo{Latin}{}{} \\
 \ea    Ab arbore abs terra pulli qui nascentur, eos in terram deprimito.\\
 \glt ‘The shoots that will grow from the tree from the ground, those you should press down into the ground.’ (Cat. \textit{agr.} 51)\\
\ex
...
\end{tabular}
\caption{From discourse functions to syntactic functions}\label{ex:F9}
\end{figure}
